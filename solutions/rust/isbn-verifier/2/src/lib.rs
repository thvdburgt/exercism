...
pub fn is_valid_isbn(isbn: &str) -> bool {
    ...
    if chars.len() != 10
        || chars[..9].iter().any(|&c| !c.is_ascii_digit())
        || !(chars[9].is_ascii_digit() || chars[9] == 'X')
    {
        return false;
    }

    // Map all chars to digits, zip it with the integer weights, compute weighted sum.
    let sum = chars
        .iter()
        .map(|&c| {
            if c == 'X' {
    ...
    // Valid if the weighted sum is divisible by 11
    sum % 11 == 0
}
